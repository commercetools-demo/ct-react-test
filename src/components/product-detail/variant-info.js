...
const VariantInfo = ({history,priceMode,variant,customer}) => {
  ...
  const callAddToCart = async () => {
    ...
    const result = await addToCart(productId,variant.id,custom,externalPrice);
    if(result) {
      history.push('/cart');
    } else {
  ...
  const getAttr = (variant,name) => {
    return variant.attributes.find(a => a.name == name);
  }

  let externalPrice;
  if(variant.price && getAttr(variant,'proratedPricing')) {
    let days = getAttr(variant,'subscription_interval_days')?.value?.key;
    console.log('days',days,variant);
    if(customer?.custom?.fields?.subscriptionEndDate && getAttr(variant,'coterminate')) {
      // Calculate a price based on per-day pricing, co-terminating with customer's existing end date
      days = Math.floor((new Date(customer.custom.fields.subscriptionEndDate) - new Date()) / (1000 * 60 * 60 * 24)) ;
    }
    externalPrice = {
      centAmount: variant.price.value.centAmount * days,
   ...
   <li>
        ...
        ?
          <div>
            { externalPrice 
              ? <span>
                  <span>Pro-rated price: {formatPrice(externalPrice)}</span><br></br>
